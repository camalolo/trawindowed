...
2. **`ShimDirect3D9`** (`ShimDirect3D9.h/.cpp`) — implements `IDirect3D9` by forwarding all calls to the real object. The only methods with custom logic:
   - `CreateDevice` — calls `trashim::initialise_shim(...)` (hooks window mode setup), forces `Windowed=TRUE`, zeroes `FullScreen_RefreshRateInHz`, and sets `PresentationInterval=IMMEDIATE` before creating the real device. Returns a `ShimDirect3DDevice9` wrapper.
...
4. **`TRAWindowed.cpp/.h`** — the core windowed-mode logic. Key responsibilities:
   ...
   - Uses **Microsoft Detours** to hook `GetCursorPos`, `SetCursorPos`, `ClipCursor`. Real pointers saved as `RealGetCursorPos` etc.
...
- **`toggle_on_top()` is inverted on first call**: The initialisation logic calls `toggle_on_top()` when `-ontop` is NOT present, meaning the default (non-on-top) state requires a toggle call. When `-ontop` IS present, no toggle is called and the window is left in its current state (not explicitly set to topmost). This is subtle and easy to break.
